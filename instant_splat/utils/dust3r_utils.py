...
from mini_dust3r.utils.image import _resize_pil_image
...
def init_from_pts3d(scene, pts3d, im_focals, im_poses):
    ...
    if nkp == 1:
        ...
        s, R, T = init_fun.align_multiple_poses(
    ...
    for e, (i, j) in enumerate(scene.edges):
        ...
        s, R, T = init_fun.rigid_points_registration(
            scene.pred_i[i_j], pts3d[i], conf=scene.conf_i[i_j]
        )
...
def init_minimum_spanning_tree(scene, focal_avg=False, known_focal=None, **kw):
    ...
    """
    device = scene.device
    pts3d, _, im_focals, im_poses = init_fun.minimum_spanning_tree(
        scene.imshapes,
        scene.edges,
        scene.pred_i,
        scene.pred_j,
        ...
        scene.min_conf_thr,
        device,
        has_im_poses=scene.has_im_poses,
...
def compute_global_alignment(
    ...
    if init is None:
        ...
        init_minimum_spanning_tree(
            scene, niter_PnP=niter_PnP, focal_avg=focal_avg, known_focal=known_focal
        )
    elif init == "known_poses":
        init_fun.init_from_known_poses(
            scene, min_conf_thr=scene.min_conf_thr, niter_PnP=niter_PnP
        )
    else:
...
def save_colmap_cameras(ori_size, intrinsics, camera_file):
    with open(camera_file, "w") as f:
        for i, K in enumerate(intrinsics, 1):  # Starting index at 1
            ...
            print(
                f"scale factor is not same for x{scale_factor_x} and y {scale_factor_y}"
            )
            f.write(
                f"{i} PINHOLE {width} {height} {K[0, 0]*scale_factor_x} {K[1, 1]*scale_factor_x} {width/2} {height/2}\n"
            )  # scale focal
...
def save_colmap_images(poses, images_file, train_img_list):
    with open(images_file, "w") as f:
        for i, pose in enumerate(poses, 1):  # Starting index at 1
            ...
            f.write(
                f"{i} {q[0]} {q[1]} {q[2]} {q[3]} {t[0]} {t[1]} {t[2]} {i} {train_img_list[i-1]}\n"
            )
            f.write(f"\n")
